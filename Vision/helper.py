...
def crop(image, top_percent, bottom_percent):
    ...
    :return:
        The cropped image
    """
    assert 0 <= top_percent < 0.6, 'top_percent should be between 0.0 and 0.5'
...
def random_shear(image, steering_angle, shear_range=170):
    """
    ...
    dsteering = dx / (rows / 2) * 360 / (2 * np.pi * 25.0) / 6
...
def generate_new_image(image, steering_angle, top_crop_percent=0.5, bottom_crop_percent=0.2,
...
def get_next_image_files(batch_size=64):
    ...
    zeros = data['x'][data['x'] == 0].sample(n=60)
    non_zeros = data['x'][data['x'] != 0].sample(n=4)
